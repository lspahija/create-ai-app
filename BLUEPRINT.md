...
- **Python 3.13+** managed with **uv** — one tool for deps, venvs, and Python versions. Lock everything with `uv.lock`. Use `pyproject.toml` for all config (deps, ruff, pytest). Use optional extras to separate concerns (e.g., `web` for FastAPI, `sdk` for AI libraries).
- **FastAPI** for the backend — async handlers, Pydantic models at API boundaries, lifespan hooks for startup/shutdown. SSE streaming via `StreamingResponse` for real-time job output.
...
- **Simple JWT auth** — optional, toggled by a single `AUTH_PASSWORD` env var. Unset = open access, set = password required. Same image for dev and prod. JWT stored in an HTTP-only cookie (not localStorage) — works with `EventSource`/SSE and is immune to XSS token theft.
